...
-- replacement for os.clock()
local function appEngine_OsClock()
  -- convert from milliseconds to seconds
  return (DateTime.getTimestamp() / 1000)
end


-- replacement for io.stdout:write(message)
local function appEngine_IoStdoutWrite(message)
  print(message)
end


-- replacement for io.stderr:write(message)
local function appEngine_IoStderrWrite(message)
  print(message)
end


-- replacement for io.stdout:flush()
local function appEngine_IoStdoutFlush()
  print(message)
end
...
math.randomseed(appEngine_OsClock()*1E11)
...
function M.wrapFunctions()
    ...
    -- so just do nothing !
    appEngine_IoStderrWrite[[Use of WrapFunctions() is no longer needed.
...
TapOutput.__class__ = 'TapOutput'
    ...
    function TapOutput:updateStatus( node )
        if node:isSkipped() then
            appEngine_IoStdoutWrite("ok ", self.result.currentTestNumber, "\t# SKIP ", node.msg, "\n" )
            return
        end

        appEngine_IoStdoutWrite("not ok ", self.result.currentTestNumber, "\t", node.testName, "\n")
    ...
    function TapOutput:endTest( node )
        if node:isSuccess() then
            appEngine_IoStdoutWrite("ok     ", self.result.currentTestNumber, "\t", node.testName, "\n")
        end
    end
...
TextOutput.__class__ = 'TextOutput'
    ...
    function TextOutput:startTest(testName)
        if self.verbosity > M.VERBOSITY_DEFAULT then
            appEngine_IoStdoutWrite( "    ", self.result.currentNode.testName, " ... " )
        end
    end
    ...
    function TextOutput:endTest( node )
        if node:isSuccess() then
            if self.verbosity > M.VERBOSITY_DEFAULT then
                appEngine_IoStdoutWrite("Ok\n")
            else
                appEngine_IoStdoutWrite(".")
                appEngine_IoStdoutFlush()
            end
        else
            if self.verbosity > M.VERBOSITY_DEFAULT then
                print( node.status )
                ...
                appEngine_IoStdoutWrite(string.sub(node.status, 1, 1))
                appEngine_IoStdoutFlush()
            end
        end
    end
...
----------------------------------------------------------------
    ...
    function M.LuaUnit:startSuite(selectedCount, nonSelectedCount)
        self.result = {
            ...
            currentNode = nil,
            suiteStarted = true,
            startTime = appEngine_OsClock(),
            --startDate = os.date(os.getenv('LUAUNIT_DATEFMT')),
            --startIsodate = os.date('%Y-%m-%dT%H:%M:%S'),

            startDate = DateTime.getDateTime(),
            startIsodate = DateTime.getDateTime(),
    ...
    function M.LuaUnit:startTest( testName  )
        ...
        self.result.currentNode.startTime = appEngine_OsClock()
    ...
    function M.LuaUnit:endTest()
        ...
        node.duration = appEngine_OsClock() - node.startTime
        node.startTime = nil
    ...
    function M.LuaUnit:endSuite()
        ...
        self.result.duration = appEngine_OsClock()-self.result.startTime
